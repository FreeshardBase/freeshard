...
def refresh_app_infra():
	...
	docker_compose_filename = Path(gconf.get('path.core')) / 'docker-compose-apps.yml'
	write_to_yaml(compose_spec(apps, portal), docker_compose_filename)

	traefik_dyn_filename = Path(gconf.get('path.core')) / 'traefik_dyn.yml'
	write_to_yaml(traefik_dyn_spec(apps, portal), traefik_dyn_filename)


def create_data_dirs(app):
	app_data_dir = Path(gconf.get('path.user_data')) / 'app_data' / app.name
