...
def _add_router(
    model: t.Model, entrypoint: Entrypoint, app: InstalledApp, portal: SafeIdentity
):
        ...
        model.http.services[f"{app.name}_{ep_value}"] = t.HttpService(
            __root__=t.HttpServiceItem(
                loadBalancer=t.HttpLoadBalancerService(
                    servers=[
                        t.Server(
                            url=f"http://{entrypoint.container_name}:{entrypoint.container_port}"
                        )
                    ]
                )
            )
        ...
        model.tcp.services[f"{app.name}_{ep_value}"] = t.TcpService(
            __root__=t.TcpServiceItem(
                loadBalancer=t.TcpLoadBalancerService(
                    servers=[
                        t.Server1(
                            address=f"{entrypoint.container_name}:{entrypoint.container_port}"
                        )
                    ]
                )
            )
