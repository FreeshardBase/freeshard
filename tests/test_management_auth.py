...
async def test_refresh_shared_secret(api_client: AsyncClient, requests_mock):
	with pytest.raises(KeyError):
		old_database.get_value(STORE_KEY_MANAGEMENT_SHARED_KEY)

	await management.refresh_shared_secret()

	assert old_database.get_value(STORE_KEY_MANAGEMENT_SHARED_KEY)
...
async def test_auth_call_success_with_wrong_shared_secret(api_client: AsyncClient,
														  requests_mock):
	old_database.set_value(STORE_KEY_MANAGEMENT_SHARED_KEY, 'wrongSecret')
	response = await api_client.get(
		'internal/authenticate_management',
		headers={'authorization': 'constantSharedSecret'}
	)
