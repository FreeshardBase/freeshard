...
async def test_add(app_client: AsyncClient):
    tour = {"name": "foo", "status": "seen"}
    put_response = await app_client.put("protected/help/tours", json=tour)
    assert put_response.status_code == 204, put_response.text

    get_response = await app_client.get("protected/help/tours/foo")
...
async def test_update(app_client: AsyncClient):
    ...
    put_response1 = await app_client.put("protected/help/tours", json=tour1)
    assert put_response1.status_code == 204, put_response1.text

    tour2 = {"name": "foo", "status": "unseen"}
    put_response2 = await app_client.put("protected/help/tours", json=tour2)
...
async def test_list(app_client: AsyncClient):
    tour1 = {"name": "foo", "status": "seen"}
    put_response1 = await app_client.put("protected/help/tours", json=tour1)
    assert put_response1.status_code == 204, put_response1.text
    ...
    get_response = await app_client.get("protected/help/tours")
    assert get_response.status_code == 200, get_response.text
...
async def test_reset(app_client: AsyncClient):
    tour = {"name": "foo", "status": "seen"}
    put_response = await app_client.put("protected/help/tours", json=tour)
    assert put_response.status_code == 204, put_response.text
